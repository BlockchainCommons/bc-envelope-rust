use crate::{known_value, IntoEnvelope};
...
#[test]
fn test_known_value() {
    let envelope = known_value::VERIFIED_BY.into_envelope().check_encoding().unwrap();
...
fn test_date() {
    ...
    let envelope = date.into_envelope().check_encoding().unwrap();
