...
use crate::{Envelope, EdgeType, FormatContext};

impl Envelope {
    pub fn tree_format(self: Rc<Envelope>, hide_nodes: bool, context: &FormatContext) -> String {
        self.tree_format_with_target(hide_nodes, &HashSet::new(), context)
    }
    
    pub fn tree_format_with_target(self: Rc<Envelope>, hide_nodes: bool, highlighting_target: &HashSet<Digest>, context: &FormatContext) -> String {
        ...
        elements.iter().map(|e| e.string(context)).collect::<Vec<_>>().join("\n")
    }
}
