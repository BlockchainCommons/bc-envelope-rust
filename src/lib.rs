...
//! ### Helpers
//!
//! * [`Envelope::signatures`] Returns an array of `Signature`s from all of the envelope's `verifiedBy` predicates.
...
//! * [`Envelope::recipients`] Returns an array of `SealedMessage`s from all of the envelope's `hasRecipient` assertions.
//! * [`Envelope::encrypt_subject_to_recipients`] Returns an new envelope with its subject encrypted and a `hasReceipient`
//! * [`Envelope::encrypt_subject_to_recipient`] Returns a new envelope with its subject encrypted and a `hasReceipient` assertion added for the `recipient`.
...
//! * [`Envelope::uncompress_subject`] Returs this envelope with its subject uncompressed.
//!
...
//! * [`Envelope::new_request`] Creates an envelope with a `CID` subject and a `body: «function»` assertion.
//! * [`Envelope::new_response`] Creates an envelope with a `CID` subject and a `result: value` assertion.
//! * [`Envelope::new_response_with_result`] Creates an envelope with a `CID` subject and a `result: value` assertion for each provided result.
//! * [`Envelope::new_error_response_with_id`] Creates an envelope with a `CID` subject and a `error: value` assertion.
