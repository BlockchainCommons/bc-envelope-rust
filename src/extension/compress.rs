...
//!
//! This module provides functionality for compressing envelopes to reduce their size
//! while maintaining their digests. Unlike elision, which removes content, compression
//! preserves all the information in the envelope but represents it more efficiently.
//!
//! Compression is implemented using the DEFLATE algorithm and preserves the envelope's
//! digest, making it compatible with the envelope's hierarchical digest tree structure.
//! This means parts of an envelope can be compressed without invalidating signatures
//! or breaking the digest tree.
//!
//! # Examples
//!
//! ```
...
impl Envelope {
    ...
    ///
    /// This method compresses the envelope using the DEFLATE algorithm, creating a more
    /// space-efficient representation while preserving the envelope's digest and semantic
    /// content. The compressed envelope maintains the same digest as the original, ensuring
    /// compatibility with the envelope's digest tree structure.
    ///
    /// When an envelope is compressed, the entire envelope structure (including its subject
    /// and assertions) is compressed as a single unit. The compression preserves all the
    /// information but reduces the size of the serialized envelope.
    ///
    /// # Returns
    ///
    ...
    ///
    /// This method reverses the compression process, restoring the envelope to its
    /// original uncompressed form. The uncompressed envelope will have the same digest
    ...
    /// assert_eq!(uncompressed.extract_subject::<String>().unwrap(), "Hello, world!");
    ...
    ///
    /// Unlike `compress()` which compresses the entire envelope, this method only
    /// compresses the subject of the envelope, leaving the assertions uncompressed.
    /// This is useful when you want to compress a large subject while keeping the
    /// assertions readable and accessible.
    ///
    /// # Returns
    ///
    ...
    ///
    /// This method reverses the effect of `compress_subject()`, uncompressing the
    /// subject of the envelope while leaving the rest of the envelope unchanged.
    ///
    /// # Returns
    ///
    /// A Result containing a new envelope with an uncompressed subject, or an error.
    ///
    /// # Errors
    ///
