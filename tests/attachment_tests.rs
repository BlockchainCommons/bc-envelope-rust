...
fn test_attachment() -> anyhow::Result<()> {
    ...
    assert_eq!(seed_envelope.attachments_with_vendor_and_conforms_to(Some("com.example"), None)?.len(), 2);
    assert_eq!(seed_envelope.attachments_with_vendor_and_conforms_to(None, Some("https://example.com/seed-attachment/v1"))?.len(), 1);
    ...
    let v1_attachment = seed_envelope.attachment_with_vendor_and_conforms_to(None, Some("https://example.com/seed-attachment/v1"))?;
