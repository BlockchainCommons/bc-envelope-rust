#![cfg(all(feature = "signature", feature = "sskr", feature = "secret", feature = "types"))]

use bc_envelope::prelude::*;
use bc_components::{ keypair, KeyDerivationMethod, SSKRGroupSpec, SSKRSpec, SymmetricKey };
use dcbor::Date;
use indoc::indoc;
...
fn test_multi_permit() -> anyhow::Result<()> {
    ...
    //
    // Alice creates a new envelope and assigns the text as the envelope's subject.
    // She also adds some metadata assertions to the envelope, including that
    // the subject is a "poem", the title, the author, and the date.
    //
    ...
    //
    // Next, Alice wants to be able to unlock her envelope using her private key,
    // and she also wants Bob to be able to unlock it using his private key. To
    // do this, she uses the `add_recipient()` method, which encrypts the content
    // key with the public keys of Alice and Bob.
    ...
    // them:
    //
    // 1. Using her original content key (usually not saved, but could be stored in a safe place)
    ...
    let unlocked_envelope = Envelope::sskr_join(
        &[&sharded_envelopes[0], &sharded_envelopes[2]]
    )?.unwrap_envelope()?;
