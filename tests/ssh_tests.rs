...
fn test_ssh_signed_plaintext() {
    ...
    let received_plaintext = received_envelope
        .verify_signature_from(&alice_ssh_public_key);
    ...
    received_envelope.verify_signatures_from_threshold(&[&alice_ssh_public_key, &carol_public_key()], Some(1)).unwrap();

    // Confirm that it was not signed by Alice AND Carol.
    assert!(received_envelope.verify_signatures_from_threshold(&[&alice_ssh_public_key, &carol_public_key()], Some(2)).is_err());
}
