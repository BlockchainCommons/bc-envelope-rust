...
fn test_known() {
    ...
    assert_eq!(envelope.format_with_context(), indoc! {r#"
    «add» [
        ❰lhs❱: 2
        ❰rhs❱: 3
    ]
...
fn test_request() {
    ...
    let request_envelope = Envelope::new_request(&request_id, two_plus_three());
        assert_eq!(request_envelope.format_with_context(), indoc! {r#"
    ...
    assert_eq!(response_envelope.format_with_context(), indoc! {r#"
    ...
    assert_eq!(error_response.format_with_context(), indoc! {r#"
        response(ARID(c66be27d)) [
    ...
    assert_eq!(unknown_error_response.format_with_context(), indoc! {r#"
        response("unknown") [
